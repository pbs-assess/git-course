...
\frame{\frametitle{Exercise 1 -- on your own}
  \bn
    \item Each person sets up a new repository \lstinline{simple-text}, following \lst{git-course-intro.pdf}.
    \item Add some notes to the readme file (e.g. just `Simple example text to learn git').
    \item Copy the text file \lst{git-course/exercise-files/simpleText.txt} into your local \lst{simple-text} repository.
    \item \lst{git commit ``<commit description>''} and \lst{git push}.
    \item Do some editing (see instructions at start of file), to get the hang of \lst{git commit} and \lst{git push}.
    \item \lst{git commit ``<commit description>''} frequently and \lst{git push} occasionally, while intermittently doing \lst{git s} to understand what's changing.
    \item Keep an eye on (and refresh) your Network Graph at \lst{https://github.com/YOUR-GITHUB-NAME/simple-text/network}\\ to understand what's going on. (There can be a short delay until it refreshes).
  \en
}
