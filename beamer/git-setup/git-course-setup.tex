...
\frame{\frametitle{\LaTeX\ installation}
  ...
  \bi
    \item On Windows, install Mik\TeX:\\
      ...
      \bigskip
      You can build this PDF by entering the \lst{beamer/git-setup} directory
      and double-click the \emph{build-pdf.bat} file.
...
\frame{\frametitle{Text editor}
  ...
  is necessary because if you merge a file together while it is open in an
  editor, you want the changes to appear and not be overwritten by what you
  had before.\\

  \bigskip

  If you use \textbf{Notepad} for editing text files, you'll need a different
  editor. A simple one that we know works is \textbf{Notepad++}. Download it
  from here (free) and install:
