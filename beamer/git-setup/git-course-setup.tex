...
\frame{\frametitle{Getting started...}
  ...
  This is a {\red one-time setup} and once it is done, you can easily create
...
\frame{\frametitle{MAC only: make your output pretty}
  On the MAC, change to the \lstinline[escapechar="\~"]{\~/github} directory
...
\frame{\frametitle{Markdown Pad}
  Each project has an associated \lstinline{README.md} file that appears
  on its \gh\ homepage. The extension \lstinline{.md} stands for Markdown
  and is just an ASCii text file that contains simple formatting (such as
  bold or italics). There are two options to read markdown files:
  \bi
  ...
  %% $^*$From \url{https://help.github.com/articles/github-glossary/}:\\
...
\frame{\frametitle{\LaTeX\ installation}
  ...
  \bi
    ...
    \item On MAC, install Mac\TeX:\\
      ...
      \bigskip
      Run the following in your shell in the directory for the given
      slideshow:\\
...
\frame{\frametitle{R and packages installation}
  ...
  \bi
    \item \lstinline{knitr}
    \item \lstinline{xtable}
  \ei

  \bigskip

  Make sure the location of \lstinline{Rscript} is included in your
  \textbf{PATH} environment variable. This is necessary to use the batch
  files that build the slides using knitr/\LaTeX.
}
...
\frame{\frametitle{To keep notes}
  ...
  \bigskip
  If so, create a file \lst{git-course/myNotes.*}, where \lst{*} is \lst{txt},
...
\frame{\frametitle{You're ready...}
  ...
  See you soon -- Wednesday 21st and Thursday 22nd June at Vancouver Island 
  Conference Centre (9am, Departure Bay Rooms A and B).
}
