...
\frame{\frametitle{Install the Git application on your machine}
  \bi
    \item Windows:\\
      ...
      without having proper permissions. If so, you may have to use a different
      computer.
...
\frame{\frametitle{Markdown Pad}
  Each repository$^*$ has an associated \lstinline{README.md} file that appears
  on the \gh\ homepage for that repository. The extension \lstinline{.md} stands
  for Markdown and is just an ASCii text file that contains simple formatting
  (such as bold or italics). To see the formatting properly it's useful to have
  an appropriate editor, such as Markdown Pad 2 which is easy to use:
...
\frame{\frametitle{\LaTeX\ installation}
  ...
  \bi
    \item On Windows, install Mik\TeX:\\
      ...
      the subdirectories inside. Each will have a \emph{build-pdf.bat} file
