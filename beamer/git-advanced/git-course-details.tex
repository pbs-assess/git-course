...
\frame{\frametitle{Commits and history}
  ...
  \bigskip
  The following slides use images from the following website:
    ...
    \begin{column}{0.4\linewidth}
      \bi
        ...
        \item Any new files you want included in the commit must be added with
          the \lstinline{git add FILENAME} command so that they are in the
    ...
    \begin{column}{0.4\linewidth}
      \bi
        \item If you have changed to another branch using
          \lstinline{git checkout maint} and then commit changes, \textbf{maint}
    ...
    \begin{column}{0.4\linewidth}
      \bi
        ...
        \item \textbf{master} and \textbf{HEAD} will be moved to the
          new commit.
        \item You can still find and use the old one if you need to:\\
          \lstinline{git reflog}
      \ei
    \end{column}
    \begin{column}{0.6\linewidth}
      \includegraphics[
    ...
    \begin{column}{0.4\linewidth}
      \bi
        ...
        \item Turn your missing commit into a new branch called
          ...
          \lstinline{git branch new SHA-1}
...
\frame{\frametitle{Stashing - 1}
  ...
  most recent one (\lstinline{stash@\{0\}}) is used:\\
  \bigskip
  \lstinline{git stash apply}
}

\frame{\frametitle{Stashing - 2}
  If you use the \lstinline{git stash apply} command, the commit which is
