...
\frame{\frametitle{From motivation talk}
  \includegraphics[
     ...
     keepaspectratio]
     {../git-motivation/figures/netWorkGraph1}\\
  This graph is for {\red Andy's repo}, so Andy is at the top.\\
  Chris's name shows up because he has pushed commits that Andy does not 
    yet have.\\
  So the graph shows that Andy should get caught up before continuing on 
    this project.\\
  ~\\
  Similarly, for hake assessment:
...
\frame{\frametitle{Network Graph}
  \bi
    \item Looked at \url{https://github.com/cgrandin/git-course/network}.\\
    \item Now see~~ \url{https://github.com/andrew-edwards/git-course/network}.\\
    \item Chris's name (probably) doesn't show up.\\
    \item But I didn't do all the work -- can see who did what by hovering 
      over commits (e.g. 16th May ones).\\
  \ei
}
