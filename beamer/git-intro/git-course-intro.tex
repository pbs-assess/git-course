...
\frame{\frametitle{Learn by doing -- Exercise 1}
  \bi
     \item Exercise 1 in \lst{git-course-exercises.pdf}
     \item Then look at your own Network Graph and we will explain it (from the \gh\ repo page that you will create, click on \lst{Insights-Graph-Network}).
     \item Click on each commit in turn on the graph. See that you (or a collaborator) can add comments.
  \ei
}
...
\frame{\frametitle{The \emph{.gitignore} file}
  ...
  \bigskip
  Anytime you run the command \lstinline{git s} and there are files listed
