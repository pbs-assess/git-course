...
\frame{\frametitle{Copy and commit \emph{.gitignore}}
  \bi
    ...
    \item Make a commit for the changes:\\
      ...
      encapsulates.\\
      Note the command \lst{com} is an alias for the more complex
...
\frame{\frametitle{Committing and pushing}
  ...
  ~\\
  \bi
  \item \lst{git com "<Message>"} commits your changes locally (Git has recorded
  ...
  Note that:
  \bi
  \item you still edit files in the usual way -- if you stop committing
    the latest versions of your files will still be there
  \item Git makes it easier to collaborate and to change to earlier versions of files
  \ei
}
...
\frame{\frametitle{Thoughts/hints regarding commit messages}
  What to write in git commit messages?\\
  \bigskip
  \centering
  \begin{figure}
    \includegraphics[
...
\frame{\frametitle{A great way to learn visually}
