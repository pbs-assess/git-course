...
\frame{\frametitle{Stashing overview}
  ...
  them back later. Let's say you are working in the master repository.To
  stash all changes:\\
  \lstinline{git stash}\\
...
\frame{\frametitle{Stashing details}
  If you leave a stash for awhile and have made changes since the stash,
  ...
  for the stash, and apply it inside the branch:\\
  \lstinline{git stash branch BRANCH-NAME}\\
  where BRANCH-NAME is the name you want to call the branch where the stash
  will be applied, typically \emph{test-stash} or something similar.
}
