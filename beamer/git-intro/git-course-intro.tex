...
\frame{\frametitle{The \emph{.gitignore} file}
  ...
  using command with wildcards such as \lstinline{git add test/*}.\\
  \bigskip
...
\frame{\frametitle{Dealing with merge conflicts}
  ...
  \bigskip
  where BRANCH-NAME is the name of the branch you are trying to merge in
...
\frame{\frametitle{Stashing - make it into a branch}
  ...
  \bigskip
  where BRANCH-NAME is the name you want to call the new branch. You can then
  merge this branch in as outlined in the merging slides and resolve conflicts
  in the same way as with remotes or other local branches.
}
...
\frame{\frametitle{Changing the commit message in the last commit}
  ...
  \bigskip
  after making the amendment to the commit message. You can also make any other
  changes you wish before issuing the amendment commit and those changes will
  be rolled into the commit you are amending.
}
...
\frame{\frametitle{Undoing a commit}
  ...
  \bigskip
  \lstinline{git revert SHA1}\\
  \bigskip
  where SHA1 is the hash for the commit you want to undo. It is the 20-byte
  string that appears with the commit.\\
  e.g.: \lstinline{1ef1da5659a4b147562b155ffb6289811adab36b}\\
  \gs\ is smart enough that you only need the first six or so digits:\\
  \bigskip
  \lstinline{git revert 1ef1da}\\
  \bigskip
...
\frame{\frametitle{Undoing changes not yet committed}
  If you've made a mess and you want to change everything back to the way
  it was on the last commit:\\
  \bigskip
...
\frame{\frametitle{A good introductory Git webpage}
