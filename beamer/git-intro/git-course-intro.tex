...
\frame{\frametitle{Dealing with merge conflicts}
  ...
  \bigskip
  where BRANCH-NAME is the name branch you are trying to merge in from the
  previously-issued command:\\
  \lstinline{git merge BRANCH-NAME}\\
  All you do is remove the bracketing lines, the ======= line, and choose
  one of the line(s) of text/code to keep, or edit the line(s) to be
  something else entirely. Once you are done fixing each conflicted file,
...
\frame{\frametitle{Dealing with merge conflicts - mergetool}
  ...
  easier in the way that it will open a new session for each conflict and
  place you at the correct place. You won't have to see the bracketing
  markers and the files will be edited by the mergetool to what you want.
  If you want to try the mergetool, after a merge conflict message, run:\\
  \lstinline{git mergetool}\\
  \bi
    \item The left panel is what was in the branch when you called the merge
      command.
    \item The middle panel is what you will end up with once finished.
    \item The right panel is what was in the branch you were trying to merge
      in.
  \ei
