...
\frame{\frametitle{Make a commit in the new branch}
  \bi
    \item Make sure you are in the new branch:\\
      \lst{git co temp}
    \item Create a new file called \lst{junk.txt}
    \item \lst{git add junk.txt}
    \item \lst{git com "Added junk.txt"}
...
\frame{\frametitle{Conflicts - what if two people have edited the same lines?}
  ...
  e.g.~\lst{git com "Kept Dave's edits as more consistent with 
  remaining text."}\\
  \lst{git push}
}
\endgroup
