...
\frame{\frametitle{Network Graph}
  \bi
     ...
     \item In the comments (or when you do \lst{git com "..."}) you can include
        a \lst{HASH} from any other commit. You only need the first five digits
        to ensure uniqueness ($16^5 = 1,048,576$ possible combinations).
  \ei
}
...
\frame{\frametitle{Adding multiple files at once - slide 1}
  ...
  \bi
    ...
    \item Add a few new test files to that directory called
      \lstinline{test1.r}, \lstinline{test2.r}, etc. Put some example text
...
\frame{\frametitle{Renaming files}
  \bi
    ...
    \item You will see that the files you renamed have been automatically
      ...
      same way you add new files (\lstinline{git add filename}). You can also
...
\frame{\frametitle{So how does Git do all that?}
  \bi
    ...
    \item Look at the \lst{objects} subdirectory. Each subdirectory name is the
      first two digits in a hash. The rest of the hash are the filenames.
    \item Git keeps track of {\red changes to files}, and so can go back and
      {\red rebuild} any file as per any \lst{commit}.
    \item Only works for ASCii (text) files, such as \lst{.r}, \lst{.txt},
      \lst{.tex}, etc.
    \item This is why you should not commit binary files; git cannot understand
      the differences between versions of them, and each one will be
      stored as a new object making your repository very large and unmanageble.
  \ei
}

\frame{\frametitle{Why you should not commit .RData etc.~files}
  An example of why not to:
  \bi
  \item A collaborator had added \lst{.pdf} and \lst{.RData} files
     (plus \lst{.r} etc.), but these got updated when the code was run.
  \item Ended up with \lst{.git/objects/pack} being 2.8Gb.
  \item Git has to save every version of each of these binary files.
