...
\frame{\frametitle{Basics}
  ...
  \bigskip
  We will use this website for some examples:
  ...
  \bn
    \item A project where people contribute to a main repository that is
      ...
      \bi
        ...
        \item Either all collaborators have push access to the main repository OR
        \item Each collaborator sends a \emph{Pull request} to the creator on the
...
\frame{\frametitle{Dealing with merge conflicts -- mergetool}
  ...
  \lst{git commit -a -m "Merge conflict resolved."}\\
  \lst{git push}
}
